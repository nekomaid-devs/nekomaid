...
export default {
    ...
    execute(command_data: CommandData) {
        ...
        const embedTransfer = {
            color: 8388736,
            description: `Transfered \`${format_number(credits_amount)} 💵\` from \`${command_data.msg.author.tag}\` to \`${command_data.tagged_user.tag}\`! (Current Credits: \`${format_number(command_data.author_user_config.credits)}$\`)`,
        };
