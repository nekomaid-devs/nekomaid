                    ...
                    case "rank": {
                        ...
                        if (command_data.args.length < 4) {
                            command_data.msg.channel
                                .send({
                                    embeds: [
                                        get_error_embed(command_data.msg, command_data.server_config.prefix, this, "You need to enter a `level_requirement`", "add rank Trusted 5 TrustedRole"),
                                    ],
                                })
                                .catch((e: Error) => {
                                    command_data.global_context.logger.api_error(e);
                        ...
                        const role = command_data.msg.guild.roles.cache.find((roleTemp) => roleTemp.name === role_name);
                        if (role === undefined) {
                            command_data.msg.channel
                                .send({
                                    embeds: [
                                        get_error_embed(command_data.msg, command_data.server_config.prefix, this, `No role with name \`${role_name}\` found`, "add rank Trusted 5 TrustedRole"),
                                    ],
                                })
                                .catch((e: Error) => {
                                    command_data.global_context.logger.api_error(e);
            ...
            case "remove": {
                ...
                switch (property) {
                    case "rank": {
                        if (command_data.args.length < 3) {
                            command_data.msg.channel
                                .send({ embeds: [get_error_embed(command_data.msg, command_data.server_config.prefix, this, "You need to enter a `rank_name`.", "remove rank Trusted")] })
                                .catch((e: Error) => {
                                    command_data.global_context.logger.api_error(e);
                                });
                            return;
                        ...
                        if (rank_ID === -1) {
                            command_data.msg.channel
                                .send({ embeds: [get_error_embed(command_data.msg, command_data.server_config.prefix, this, `No rank with name \`${rank_name}\` found`, "remove rank Trusted")] })
                                .catch((e: Error) => {
                                    command_data.global_context.logger.api_error(e);
                                });
                            return;
                    ...
                    case "level_exp": {
                        if (isNaN(parseFloat(value)) || parseFloat(value) <= 0) {
                            command_data.msg.channel
                                .send({
                                    embeds: [
                                        get_error_embed(command_data.msg, command_data.server_config.prefix, this, `Invalid value to set for \`${property}\`. (number)`, `set ${property} 200`),
                                    ],
                                })
                                .catch((e: Error) => {
                                    command_data.global_context.logger.api_error(e);
                    ...
                    case "level_multiplier": {
                        if (isNaN(parseFloat(value)) || parseFloat(value) <= 0) {
                            command_data.msg.channel
                                .send({
                                    embeds: [
                                        get_error_embed(command_data.msg, command_data.server_config.prefix, this, `Invalid value to set for \`${property}\`. (number)`, `set ${property} 1.3`),
                                    ],
                                })
                                .catch((e: Error) => {
                                    command_data.global_context.logger.api_error(e);
