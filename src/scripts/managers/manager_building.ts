...
class BuildingManager {
    ...
    async update_buildings(global_context: GlobalContext, user: GlobalUserData) {
        ...
        if (user.b_lewd_services > 0 && diff >= 60) {
            ...
            credits_amount = credits_amount * (global_context.bot_config.shrine_bonus === ShrineBonus.HOURLY ? [1, 1, 1, 1, 1.01, 1.02, 1.03, 1.05, 1.05, 1.07, 1.1][global_context.bot_config.b_shrine] : 1);
        ...
        if (user.b_pawn_shop > 0) {
            ...
            if (user.b_pawn_shop >= 8 && l_items.length > 0) {
                if (diff >= [0, 0, 0, 0, 0, 0, 0, 0, 60 * 6, 60 * 6, 60 * 6][user.b_pawn_shop]) {
                    ...
                    credits_amount = credits_amount * (global_context.bot_config.shrine_bonus === ShrineBonus.SELLS ? [1, 1, 1, 1, 1, 1, 1, 1.05, 1.1, 1.15, 1.15][global_context.bot_config.b_shrine] : 1);
                    credits_amount = Math.round(credits_amount);
