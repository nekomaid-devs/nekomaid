    ...
    switch (data.type) {
        ...
        case "server_channel_create":
            return await fetch_data(
                global_context,
                "SELECT server_ID, mute_role_ID FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
                    return await global_context.neko_modules_clients.mySQL.mysql_add.add_server(global_context, { id: data.id });
                }
            );
        ...
        case "server_guild_ban_remove":
            return await fetch_data(
                global_context,
                "SELECT server_ID, audit_channel, audit_bans, case_ID FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_guild_member_clear_warns":
            return await fetch_data(
                global_context,
                "SELECT server_ID, audit_channel, audit_warns, case_ID FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_guild_member_mute_ext":
            return await fetch_data(
                global_context,
                "SELECT server_ID, audit_channel, audit_mutes, case_ID FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_guild_member_add":
            return await fetch_data(
                global_context,
                "SELECT server_ID, auto_roles, mute_role_ID, welcome_messages, welcome_messages_format, welcome_messages_channel, welcome_messages_ping FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_guild_member_remove":
            return await fetch_data(
                global_context,
                "SELECT server_ID, leave_messages, leave_messages_format, leave_messages_channel, audit_channel, audit_kicks FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_message_delete":
            return await fetch_data(
                global_context,
                "SELECT server_ID, audit_channel, audit_deleted_messages FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_message_update":
            return await fetch_data(
                global_context,
                "SELECT server_ID, audit_channel, audit_edited_messages FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_guild_member_nickname_update":
            return await fetch_data(
                global_context,
                "SELECT server_ID, audit_channel, audit_nicknames FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_message":
            return await fetch_data(
                global_context,
                "SELECT server_ID, prefix, banned_words, invites, module_level_enabled, module_level_message_exp FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server":
            return await fetch_data(
                global_context,
                "SELECT * FROM servers WHERE server_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_server(global_context, e, data.containExtra, data.containRanks);
                },
                async () => {
        ...
        case "server_user": {
            const fast_find_ID = data.server_ID + "-" + data.user_ID;
            return await fetch_data(global_context, "SELECT * FROM server_users WHERE fast_find_ID='" + fast_find_ID + "'", defaultFormat, async () => {
                return await global_context.neko_modules_clients.mySQL.mysql_add.add_server_user(global_context, { id: data.server_ID }, { id: data.user_ID });
            });
        }

        case "all_server_users":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_users WHERE server_ID='" + data.id + "'", defaultFormat);

        case "counters":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_counters WHERE server_ID='" + data.id + "'", defaultFormat);

        case "all_counters":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_counters", defaultFormat);

        case "ranks":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_ranks WHERE server_ID='" + data.id + "'", defaultFormat);

        case "reaction_roles":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_reaction_roles WHERE server_ID='" + data.id + "'", format_reaction_role);

        case "all_reaction_roles":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_reaction_roles", format_reaction_role);

        case "global_user":
            return await fetch_data(
                global_context,
                "SELECT * FROM global_users WHERE user_ID='" + data.id + "'",
                async (e: any) => {
                    return await format_global_user(global_context, e, true, true);
                },
                async () => {
                    return await global_context.neko_modules_clients.mySQL.mysql_add.add_global_user(global_context, { id: data.id });
                }
            );

        case "all_global_users":
            return await fetch_multiple_data(global_context, "SELECT * FROM global_users", async (e: any) => {
                return await format_global_user(global_context, e);
            });

        case "buildings_global_users":
            return await fetch_multiple_data(global_context, "SELECT * FROM global_users WHERE b_lewd_services > 0 AND b_casino > 0 AND b_scrapyard > 0 AND b_pawn_shop > 0", async (e: any) => {
                return await format_global_user(global_context, e, true, true);
            });

        case "server_bans":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_bans WHERE server_ID='" + data.id + "'", defaultFormat);

        case "all_server_bans":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_bans", defaultFormat);

        case "expired_server_bans":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_bans WHERE end <> -1 AND end < " + data.time, defaultFormat);

        case "server_mutes":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_mutes WHERE server_ID='" + data.id + "'", defaultFormat);

        case "all_server_mutes":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_mutes", defaultFormat);

        case "expired_server_mutes":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_mutes WHERE end <> -1 AND end < " + data.time, defaultFormat);

        case "server_warnings":
            return await fetch_multiple_data(global_context, "SELECT * FROM server_warnings WHERE server_ID='" + data.id + "'", defaultFormat);

        case "inventory_items":
            return await fetch_multiple_data(global_context, "SELECT * FROM inventory_items WHERE user_ID='" + data.id + "'", defaultFormat);

        case "user_notifications":
            return await fetch_multiple_data(global_context, "SELECT * FROM user_notifications WHERE user_ID='" + data.id + "'", defaultFormat);

        case "server_logs":
            /*return await fetch_data(bot, "SELECT * FROM serverLogs WHERE server_ID='" + data.id + "'", defaultFormat);*/
            return global_context.neko_modules_clients.mySQL.server_prefabs.getServerLogsPrefab({ id: data.id });
    }
...
async function fetch_data(global_context: GlobalContext, query: string, formattingFunc: any, creatingFunc: any) {
    ...
    if (result.length < 1 || result[0].length < 1) {
        await creatingFunc();
        result = await global_context.neko_modules_clients.mySQL.sql_connection.promise().query(query);
        if (result.length < 1 || result[0].length < 1) {
...
async function format_server(global_context: GlobalContext, server: any, containExtra = false, containRanks = false) {
    ...
    if (containExtra === true || containRanks === true) {
        server.module_level_ranks = await fetch(global_context, { type: "ranks", id: server.server_ID });
    }

    return server;
}

async function format_global_user(global_context: GlobalContext, user: any, containInventory = false, containNotifications = false) {
    if (containInventory === true) {
        user.inventory = await fetch(global_context, { type: "inventory_items", id: user.user_ID });
    }
    if (containNotifications === true) {
        user.notifications = await fetch(global_context, { type: "user_notifications", id: user.user_ID });
    }
