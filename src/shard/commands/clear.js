...
module.exports = {
    ...
    async execute(command_data) {
        ...
        if (command_data.msg.mentions.users.size > 0) {
            let target_user = command_data.msg.mentions.users.first();
            let messages = await command_data.msg.channel.messages.fetch({ limit: 99 }).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
            messages = Array.from(messages.values());

            messages.pop();
            let target_messages = messages.filter((m) => m.author.id === target_user.id);
            target_messages = target_messages.slice(target_messages.length - num_messages, target_messages.length + 1);

            command_data.msg.channel.bulkDelete(1, true).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
