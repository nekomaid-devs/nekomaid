module.exports = {
    ...
    async execute(command_data) {
            ...
            case 0: {
                ...
                for (let i = 1; i <= 5; i += 1) {
                    if (voice_data.queue.length >= i) {
                        ...
                        let user = await command_data.global_context.bot.users.fetch(voice_request.request_user_ID).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });

                        if (user !== undefined) {
                            let current_length = command_data.global_context.neko_modules_clients.tc.convert_time_data_to_string(
                                command_data.global_context.neko_modules_clients.tc.convert_youtube_string_to_time_data(voice_request.info.duration)
                            );
            ...
            case 1: {
                ...
                for (let i2 = start; i2 <= start + end; i2++) {
                    if (voice_data.persistent_queue.length > i2) {
                        ...
                        let user2 = await command_data.global_context.bot.users.fetch(voice_request_2.request_user_ID).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        ...
                        if (user2 !== undefined) {
                            ...
                            let current_length = command_data.global_context.neko_modules_clients.tc.convert_time_data_to_string(
                                command_data.global_context.neko_modules_clients.tc.convert_youtube_string_to_time_data(voice_request_2.info.duration)
                            );
                ...
                let current_length_b = command_data.global_context.neko_modules_clients.tc.convert_time_data_to_string(
                    command_data.global_context.neko_modules_clients.tc.convert_youtube_string_to_time_data(voice_data.persistent_queue[current_persistent_index].info.duration)
                );
        ...
        command_data.msg.channel.send({ embeds: [embedQueue] }).catch((e) => {
            command_data.global_context.logger.api_error(e);
        });
    },
};
