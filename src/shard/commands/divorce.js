module.exports = {
    ...
    async execute(command_data) {
        if (command_data.msg.mentions.users.size > 0) {
            ...
            let married_user = await command_data.global_context.bot.users.fetch(command_data.tagged_user_config.married_ID).catch((e) => {
            ...
            if (married_user === undefined) {
                ...
                command_data.global_context.neko_modules_clients.ssm.server_edit.edit(command_data.global_context, { type: "global_user", user: tagged_user_config });

                user_config.married_ID = "-1";
                user_config.can_divorce = true;
                command_data.global_context.neko_modules_clients.ssm.server_edit.edit(command_data.global_context, { type: "global_user", user: user_config });

                command_data.msg.channel.send(`Force divorced \`${command_data.tagged_user.tag}\` and \`${married_user.tag}\`!`).catch((e) => {
                    command_data.global_context.logger.api_error(e);
                });
            }
        ...
        let married_user = await command_data.global_context.bot.users.fetch(command_data.author_config.married_ID).catch((e) => {
            command_data.global_context.logger.api_error(e);
        });
        let user_config = -1;
        if (married_user !== undefined) {
        ...
        command_data.msg.channel.send(`\`${command_data.msg.author.tag}\` divorced \`${married_user.tag}\`!`).catch((e) => {
            command_data.global_context.logger.api_error(e);
        });
    },
};
