...
module.exports = {
    ...
    async execute(command_data) {
        ...
        if (a === 0 || a % 2 != 0) {
            command_data.msg.channel.send("Check your syntax before trying to create a poll again!").catch((e) => {
        ...
        if (parts.length > 11) {
            command_data.msg.channel.send("Maximum number of answers is `10`!").catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
            return;
        ...
        let description = parts.reduce((acc, curr, i) => {
            acc += "**" + (i + 1) + ")** " + curr + "\n";
            return acc;
        }, "");
        ...
        let message = await command_data.msg.channel.send({ embeds: [embedPoll] }).catch((e) => {
            command_data.global_context.logger.api_error(e);
        });
        let reactions = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"];
        parts.forEach((part, i) => {
            message.react(reactions[i]).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
        });
    },
