...
module.exports = {
    ...
    async execute(command_data) {
        ...
        let embedTop = new command_data.global_context.modules.Discord.MessageEmbed().setColor(8388736).setTitle(`❯    Top - \`${top_text}\` ${top_text_2}`);
        ...
        for (let i = 0; i < limit; i += 1) {
            ...
            let net = props.reduce((acc, curr) => {
                acc += user_config[curr];
                return acc;
            }, 0);
            let target_user = await command_data.global_context.bot.users.fetch(user_config.user_ID).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
            embedTop.addField(`${i + 1}) ${target_user.tag}`, `${command_data.global_context.utils.format_number(net)} ${top_user_text}`);
        }

        command_data.msg.channel.send({ embeds: [embedTop] }).catch((e) => {
            command_data.global_context.logger.api_error(e);
        });
    },
};
