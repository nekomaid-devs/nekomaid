...
module.exports = {
    ...
    async execute(command_data) {
        let msg = await command_data.msg.channel.send("Type in a role name or `stop` to finish the menu!").catch((e) => {
    ...
    continue_collecting(global_context, server_config, source_message, msg, role_messages, roles) {
        let filter = (message) => message.author.id === source_message.author.id;
            ...
            switch (message.content) {
                case "stop": {
                    if (roles.size < 1) {
                        ...
                        msg.edit("Cancelled the reaction menu.").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        return;
                    }
                    ...
                    let reactionRoleEmbed = new global_context.modules.Discord.MessageEmbed().setColor(8388736).setTitle(`Roles (${roles.size})`);
                    ...
                    msg.edit({ embeds: [reactionRoleEmbed] }).catch((e) => {
                        global_context.logger.api_error(e);
                    });
                ...
                default: {
                    ...
                    let filter_2 = (reaction, user) => user.id === source_message.author.id;
                    let collector_2 = message.createReactionCollector({ filter: filter_2, max: 1 });
