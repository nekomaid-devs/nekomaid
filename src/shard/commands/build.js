...
module.exports = {
    ...
    execute(command_data) {
        let amount = parseInt(command_data.args[command_data.args.length - 1]);
        let building_name = isNaN(amount) ? command_data.total_argument : command_data.args.slice(0, command_data.args.length - 1).join(" ");
        ...
        if (building_field !== undefined) {
            if (isNaN(amount)) {
                ...
                command_data.msg.channel.send({ embeds: [embedBuild] }).catch((e) => {
                ...
                if (command_data.author_config.credits - amount < 0) {
                    command_data.msg.reply(`You don't have enough credits to do this.`);
                    return;
                }

                command_data.author_config.credits -= amount;
                command_data.author_config[building_field + "_credits"] += amount;
                ...
                command_data.msg.channel.send({ embeds: [embedBuildProgress] }).catch((e) => {
                    command_data.global_context.logger.api_error(e);
                });
            }
        }
