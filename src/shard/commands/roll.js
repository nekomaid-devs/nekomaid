...
module.exports = {
    ...
    async execute(command_data) {
        ...
        if (command_data.args.length > 2) {
            ...
            let message = await command_data.msg.channel.send({ embeds: [embedRoll] }).catch((e) => {
            ...
            setTimeout(() => {
                ...
                if (result === bet_result) {
                    ...
                    let won_amount = Math.floor(credits_amount * multiplier);
                ...
                message.edit({ embeds: [embedRoll] }).catch((e) => {
                    command_data.global_context.logger.api_error(e);
                });
            }, 750);
        } else {
            let result = command_data.global_context.utils.pick_random(options);
            embedRoll.title = `${command_data.msg.author.tag} rolled ${result}!`;
            command_data.msg.channel.send({ embeds: [embedRoll] }).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
        }
    },
