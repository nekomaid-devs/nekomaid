    ...
    async execute(command_data) {
        ...
        if (command_data.args.length < 1) {
            ...
            for (let i = 0; i < command_data.server_config.auto_roles.length; i++) {
                ...
                let role = await command_data.msg.guild.roles.fetch(role_ID).catch((e) => {
            ...
            command_data.msg.channel.send({ embeds: [embedConfig] }).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
            return;
        }
        ...
        switch (action) {
            case "add": {
                ...
                switch (property) {
                    case "auto_role": {
                        ...
                        if (command_data.args.length < 3) {
                            command_data.msg.channel
                                .send({ embeds: [command_data.global_context.neko_modules.vars.get_error_embed(command_data.msg, command_data.server_config.prefix, this, "You need to enter a `roleName`.", "add auto_role Newbie")] })
                                .catch((e) => {
                                    command_data.global_context.logger.api_error(e);
                                });
                            return;
                        ...
                        let role = command_data.msg.guild.roles.cache.find((role_temp) => role_temp.name === role_name);
                        if (role === undefined) {
                            command_data.msg.channel
                                .send({
                                    embeds: [command_data.global_context.neko_modules.vars.get_error_embed(command_data.msg, command_data.server_config.prefix, this, `No role with name .\`${role_name}\` found-`, "add auto_role Newbie")],
                                })
                                .catch((e) => {
                                    command_data.global_context.logger.api_error(e);
                                });
                        ...
                        command_data.msg.channel.send(`Added auto-role \`${role_name}\`.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "counter": {
                        ...
                        setTimeout(() => {
                            command_data.global_context.neko_modules_clients.cm.update_counter(command_data.global_context, command_data.msg.guild, counter, true);
                        }, 5000);
                        command_data.msg.channel.send(`Added new counter, wait for it to load.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                ...
                switch (property) {
                    case "auto_role": {
                        ...
                        if (role === undefined) {
                            command_data.msg.channel
                                .send({
                                    embeds: [command_data.global_context.neko_modules.vars.get_error_embed(command_data.msg, command_data.server_config.prefix, this, `No role with name \`${role_name}\` found-`, "remove auto_role Newbie")],
                                })
                                .catch((e) => {
                                    command_data.global_context.logger.api_error(e);
                                });
                        ...
                        command_data.msg.channel.send(`Removed auto-role \`${role_name}\`.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
            ...
            case "set": {
                ...
                switch (property) {
                    case "say_command": {
                        ...
                        command_data.msg.channel.send(`${bool ? "Enabled" : "Disabled"} the say command.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "welcome_messages": {
                        ...
                        command_data.msg.channel.send(`${bool ? "Enabled" : "Disabled"} welcome messages.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "welcome_messages_format": {
                        ...
                        command_data.msg.channel.send(`Edited the welcome messages format.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "leave_messages": {
                        ...
                        command_data.msg.channel.send(`${bool ? "Enabled" : "Disabled"} leave messages.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "leave_messages_format": {
                        ...
                        command_data.msg.channel.send(`Edited the leave messages format.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "welcome_messages_channel": {
                        ...
                        command_data.msg.channel.send(`Set welcome messages channel to <#${value}>.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "leave_messages_channel": {
                        ...
                        command_data.msg.channel.send(`Set leave messages channel to <#${value}>.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "welcome_messages_ping": {
                        ...
                        command_data.msg.channel.send(`${bool ? "Enabled" : "Disabled"} pings in welcome messages.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
            ...
            default: {
                command_data.msg.channel
                    .send({
                        embeds: [command_data.global_context.neko_modules.vars.get_error_embed(command_data.msg, command_data.server_config.prefix, this, "Invalid action- (Actions: `add`, `set`, `remove`)", "set welcome_messages true")],
                    })
                    .catch((e) => {
                        command_data.global_context.logger.api_error(e);
                    });
        ...
        if (command_data.server_config.welcome_messages == true && command_data.server_config.welcome_messages_channel === "-1") {
            command_data.msg.channel.send("Make sure to set `welcome_messages_channel`, otherwise welcome messages won't work.").catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
        }

        if (command_data.server_config.leave_messages == true && command_data.server_config.leave_messages_channel === "-1") {
            command_data.msg.channel.send("Make sure to set `leave_messages_channel`, otherwise leave messages won't work.").catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
        }
    },
