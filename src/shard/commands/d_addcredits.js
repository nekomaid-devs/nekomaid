...
module.exports = {
    ...
    execute(command_data) {
        ...
        command_data.msg.channel
            .send(
                `Added \`${command_data.global_context.utils.format_number(credits_amount)}\` credits to \`${command_data.tagged_user.tag}\`! (Current credits: \`${command_data.global_context.utils.format_number(
                    command_data.tagged_user_config.credits
                )}$\`)`
            )
            .catch((e) => {
