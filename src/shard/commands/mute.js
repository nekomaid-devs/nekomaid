...
module.exports = {
    ...
    permissionsNeeded: [new NeededPermission("author", Permissions.FLAGS.BAN_MEMBERS), new NeededPermission("me", Permissions.FLAGS.ANAGE_ROLES), new NeededPermission("me", Permissions.FLAGS.MANAGE_CHANNELS)],
    nsfw: false,
    cooldown: 1500,
    async execute(command_data) {
        ...
        if (previous_mute === -1) {
            ...
            command_data.msg.channel.send(`Muted \`${command_data.tagged_user.tag}\` for \`${extended_time_text}\` (Reason: \`${mute_reason}\`, Time: \`${mute_end_text}\`)-`).catch((e) => {
            ...
            command_data.msg.channel.send(`Extended mute of \`${command_data.tagged_user.tag}\` by \`${extended_time_text}\` (Reason: \`${mute_reason}\`, Time: \`${mute_end_text}\`)-`).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
        ...
        if (command_data.server_config.mute_role_ID === "-1") {
            ...
            let mute_role = await command_data.msg.guild.roles.fetch(command_data.server_config.mute_role_ID).catch((e) => {
                command_data.global_context.logger.api_error(e);
            });
