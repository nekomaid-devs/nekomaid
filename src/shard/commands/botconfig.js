    ...
    async execute(command_data) {
        ...
        if (command_data.args.length < 1) {
            ...
            for (let i = 0; i < command_data.global_context.bot_config.bot_owners.length; i++) {
                ...
                let owner = await command_data.global_context.bot.users.fetch(owner_ID).catch((e) => {
        ...
        switch (action) {
            case "add": {
                ...
                if (command_data.msg.mentions.users.size > 0) {
                    tagged_users = [...command_data.msg.mentions.users.values()];
                } else {
                ...
                switch (property) {
                    case "bot_owner": {
                        ...
                        command_data.msg.channel.send(`Added \`${tagged_user.tag}\` as a bot owner.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
            ...
            case "set": {
                ...
                switch (property) {
                    case "speed": {
                        ...
                        command_data.msg.channel.send(`Set speed to \`${value.toFixed(2)}\`.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
            ...
            case "remove": {
                ...
                switch (property) {
                    ...
                    case "bot_owner": {
                        ...
                        command_data.msg.channel.send(`Removed \`${tagged_user.tag}\` from bot owners.`).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                        break;
                    }
            ...
            default: {
                command_data.msg.channel
                    .send({
                        embeds: [command_data.global_context.neko_modules.vars.get_error_embed(command_data.msg, command_data.server_config.prefix, this, "Invalid action- (Actions: `add`, `set`, `remove`)", "add bot_owner @LamkasDev")],
                    })
                    .catch((e) => {
                        command_data.global_context.logger.api_error(e);
                    });
