...
module.exports = {
    ...
    async execute(command_data) {
        ...
        let num_of_warnings = command_data.server_warns.filter((warn) => warn.user_ID === command_data.tagged_user.id).length;
        command_data.msg.channel.send(`Warned \`${command_data.tagged_user.tag}\`. (Reason: \`${warn_reason}\`, Strikes: \`${num_of_warnings}\` => \`${num_of_warnings + 1}\`)`).catch((e) => {
