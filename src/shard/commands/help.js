    ...
    execute(command_data) {
        ...
        if (command_data.args.length > 0) {
            ...
            if (command_keys.includes(target_command_name) === false || (command_data.global_context.commands.get(target_command_name).hidden === true && show_hidden === false)) {
                command_data.msg.channel.send(`Command \`${target_command_name}\` not found - see \`${command_data.server_config.prefix}help\` for help`).catch((e) => {
            ...
            if (command_data.args.length > 1) {
                ...
                if (command.subcommandHelp.has(target_subcommand_name) === false) {
                    command_data.msg.channel.send(`Sub-command \`${target_subcommand_name}\` not found - see \`${command_data.server_config.prefix}help\` for help`).catch((e) => {
                        command_data.global_context.logger.api_error(e);
                    });
                    return;
                }
                ...
                let embedHelp = new command_data.global_context.modules.Discord.MessageEmbed().setColor(8388736).setTitle(`Help for - \`${unhidden_text + commands_text}\``);
                embedHelp.addField("Usage:", usage);

                command_data.msg.channel.send({ embeds: [embedHelp] }).catch((e) => {
                    command_data.global_context.logger.api_error(e);
                });
            } else {
