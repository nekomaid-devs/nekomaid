...
module.exports = {
    ...
    argumentsNeeded: [new NeededArgument(1, "You need to mention somebody.", "mention"), new NeededArgument(2, "You need to type in an amount.", "float>0", "Invalid XP amount!")],
    ...
    execute(command_data) {
        ...
        command_data.global_context.neko_modules_clients.lvl.update_server_level(command_data, add_XP);

        command_data.msg.channel.send(`Added \`${add_XP}\` XP to \`${command_data.tagged_user.tag}\`! (Current XP: \`${Math.round(command_data.taggedServerUserConfig.xp)}\`)`).catch((e) => {
