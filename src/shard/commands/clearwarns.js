...
module.exports = {
    ...
    async execute(command_data) {
        ...
        let warns = command_data.server_warns.filter((warn) => warn.user_ID === command_data.tagged_user.id);
        command_data.msg.channel.send(`Cleared warnings of \`${command_data.tagged_user.tag}\`. (Reason: \`${warn_reason}\`, Strikes: \`${warns.length}\` => \`0\`)-`).catch((e) => {
