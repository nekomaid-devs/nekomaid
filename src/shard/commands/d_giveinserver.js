...
module.exports = {
    ...
    async execute(command_data) {
        let amount = parseInt(command_data.args[0]);
        let item_ID = command_data.args[1];
        let target_item = Array.from(command_data.global_context.bot_config.items.values()).find((e) => {
            return e.id === item_ID;
        });
        ...
        command_data.msg.channel.send(`Added \`${amount}x ${target_item.display_name}\` to \`${command_data.msg.guild.members.cache.size}\` members!`).catch((e) => {
