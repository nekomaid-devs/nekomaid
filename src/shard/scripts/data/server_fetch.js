module.exports = {
        ...
        switch (data.type) {
            ...
            case "server_channel_create":
                return await this.fetch_data(
                    ...
                    async (e) => {
                        return await this.format_server(global_context, e, data.containExtra, data.containRanks);
                    },
                    async () => {
                        return await global_context.neko_modules_clients.ssm.server_add.add_server(global_context, { id: data.id });
                    }
                );
            ...
            case "server_user": {
                ...
                return await this.fetch_data(global_context, "SELECT * FROM server_users WHERE fast_find_ID='" + fast_find_ID + "'", defaultFormat, async () => {
                    return await global_context.neko_modules_clients.ssm.server_add.add_server_user(global_context, { id: data.server_ID }, { id: data.user_ID });
                });
            }
            ...
            case "global_user":
                return await this.fetch_data(
                    ...
                    async (e) => {
                        return await this.format_global_user(global_context, e, true, true);
                    },
                    async () => {
                        return await global_context.neko_modules_clients.ssm.server_add.add_global_user(global_context, { id: data.id });
                    }
                );

            case "all_global_users":
                return await this.fetch_multiple_data(global_context, "SELECT * FROM global_users", async (e) => {
                    return await this.format_global_user(global_context, e);
                });

            case "buildings_global_users":
                return await this.fetch_multiple_data(global_context, "SELECT * FROM global_users WHERE b_lewd_services > 0 AND b_casino > 0 AND b_scrapyard > 0 AND b_pawn_shop > 0", async (e) => {
                    return await this.format_global_user(global_context, e, true, true);
                });
    ...
    async fetch_multiple_data(global_context, query, formattingFunc) {
        ...
        result = result[0].reduce((acc, curr) => {
