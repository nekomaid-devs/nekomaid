class CounterManager {
    async update_all_counters(global_context) {
        ...
        global_context.bot.guilds.cache.forEach((server) => {
            let server_counters = counters.filter((e) => {
    ...
    async update_counter(global_context, server, counter, force_update = false) {
        ...
        if (diff >= 5 || force_update === true) {
            ...
            let channel = await global_context.bot.channels.fetch(counter.channel_ID).catch((e) => {
            ...
            if (channel !== undefined) {
                switch (counter.type) {
                    case "all_members": {
                        ...
                        channel.setName(`All Members: ${member_count}`).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "members": {
                        await global_context.utils.verify_guild_members(server);
                        let member_count = Array.from(server.members.cache.values()).filter((e) => {
                            return e.user.bot === false;
                        }).length;
                        channel.setName(`Members: ${member_count}`).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "roles": {
                        ...
                        channel.setName(`Roles: ${roles_count}`).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "bots": {
                        await global_context.utils.verify_guild_members(server);
                        let bot_count = Array.from(server.members.cache.values()).filter((e) => {
                            return e.user.bot === false;
                        }).length;
                        channel.setName(`Bots: ${bot_count}`).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "bot_servers": {
                        ...
                        channel.setName(`Current Servers: ${guild_count}`).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        break;
                    }
                    ...
                    case "bot_users": {
                        ...
                        channel.setName(`Current Users: ${member_count}`).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        break;
                    }
