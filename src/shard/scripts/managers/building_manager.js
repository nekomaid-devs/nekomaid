class BuildingManager {
    ...
    async update_buildings(global_context, user) {
        ...
        if (user.b_pawn_shop > 0) {
            let l_items = user.inventory.filter((e) => {
                let item = global_context.bot_config.items.get(e.item_ID);
                return item.rarity === "legendary" && item.can_be_scavanged === true;
            });
            let r_items = user.inventory.filter((e) => {
                let item = global_context.bot_config.items.get(e.item_ID);
                return item.rarity === "rare" && item.can_be_scavanged === true;
            });
            let u_items = user.inventory.filter((e) => {
                let item = global_context.bot_config.items.get(e.item_ID);
                return item.rarity === "uncommon" && item.can_be_scavanged === true;
            });
            let c_items = user.inventory.filter((e) => {
                let item = global_context.bot_config.items.get(e.item_ID);
