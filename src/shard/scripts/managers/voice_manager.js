class VoiceManager {
    ...
    async remove_connection(global_context, id, error_message = -1) {
        ...
        if (global_context.neko_modules_clients.vm.connections.get(id).current !== -1) {
            ...
            if (error_message !== -1) {
                let guild = await global_context.bot.guilds.fetch(id).catch((e) => {
                    global_context.logger.api_error(e);
                });
                if (guild !== undefined) {
                    let channel = await guild.channels.fetch(global_context.neko_modules_clients.vm.connections.get(id).current.request_channel_ID).catch((e) => {
                        global_context.logger.api_error(e);
                    });
                    if (channel !== undefined) {
                        channel.send(error_message).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    }
                }
    ...
    async timeout_connection(global_context, id) {
        ...
        if (voice_data.connection.channel.members.size <= 1) {
            ...
            if (guild !== undefined) {
                let channel = await guild.channels.fetch(voice_data.init_message_channel_ID).catch((e) => {
                    global_context.logger.api_error(e);
                });
                if (channel !== undefined) {
                    channel.send(`I left \`${voice_data.connection.channel.name}\`, because I was left alone.`).catch((e) => {
                        global_context.logger.api_error(e);
                    });
                }
            }
    ...
    async play_on_connection(global_context, source_message, loading_message, info, log_type) {
        ...
        if (current_length.status !== -1 && (current_length.hrs >= 3 || info.duration === "P0D") && diff > 3600) {
            ...
            await source_message.channel.send({ embeds: [embedPlay] }).catch((e) => {
                global_context.logger.api_error(e);
            });
            return;
        }
        ...
        if (voice_data.current === -1) {
            ...
            stream.on("error", (err) => {
                source_message.channel.send("There was an error while playing the video...").catch((e) => {
                    global_context.logger.api_error(e);
                });
        ...
        if (global_context.neko_modules_clients.vm.connections.has(id) === true) {
            ...
            if (voice_data.queue.length > 0) {
                ...
                stream.on("error", async (err) => {
                    let channel = await global_context.bot.channels.fetch(voice_request.request_channel_ID).catch((e) => {
                        global_context.logger.api_error(e);
                    });
                    if (channel !== undefined) {
                        channel.send("There was an error while playing the video...").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    }
                ...
                if (channel !== undefined) {
                    ...
                    channel.send({ embeds: [embedPlay] }).catch((e) => {
                        global_context.logger.api_error(e);
                    });
                }
