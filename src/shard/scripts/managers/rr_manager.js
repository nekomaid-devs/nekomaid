class ReactionRolesManager {
    async create_all_collectors(global_context) {
        ...
        global_context.bot.guilds.cache.forEach((server) => {
            let server_reaction_roles = reaction_roles.filter((e) => {
    ...
    async create_collector(global_context, server, rr) {
        let channel = await global_context.bot.channels.fetch(rr.channel_ID).catch((e) => {
            global_context.logger.api_error(e);
        });
        if (channel !== undefined) {
            let message = await channel.messages.fetch(rr.message_ID).catch((e) => {
                global_context.logger.api_error(e);
            });
            if (message !== undefined) {
                let filter = (r, u) => u.bot === false;
                let collector = message.createReactionCollector({ filter: filter, dispose: true });
                collector.on("collect", (r, user) => {
                    rr.reaction_roles.forEach(async (role_ID, i) => {
                        ...
                        if (emoji === r.emoji.name || (r.emoji.id !== undefined && emoji === "<:" + r.emoji.name + ":" + r.emoji.id + ">")) {
                            ...
                            if (member !== undefined) {
                                let role = Array.from(server.roles.cache.values()).find((e) => {
                                    return e.id === role_ID;
                                });
                                if (role !== undefined) {
