class EventManager {
    async spawn_event(global_context, channelID, _type) {
        if (global_context.config.events_enabled === false) {
            return;
        }

        let channel = await global_context.bot.channels.fetch(channelID).catch((e) => {
        ...
        switch (type) {
            ...
            case 7:
                ...
                setTimeout(() => {
                    ...
                    let amount = data.joined_users.length * 100;
                    if (c > 40) {
                        ...
                        channel.send("The robbery was a failure and everyone except the neko was caught~ Everyone's reputation lowered~").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    }
                }, remaining_time);
            ...
            case 11:
                ...
                setTimeout(async () => {
                    if (data.tickets.length > 0) {
                        ...
                        global_context.neko_modules_clients.ssm.server_edit.edit(global_context, { type: "global_user", user: winner_config }).catch((e) => {
                            global_context.logger.api_error(e);
                        });

                        channel.send("<@" + winner_ID + "> won the prize of 2000$ from total of " + data.tickets.length + " tickets!~").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    } else {
                        channel.send("Nobody bought any tickets and the neko sadly wanders away~").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    }
                }, remaining_time);
        ...
        let message = await channel.send({ embed: embed }).catch((e) => {
            global_context.logger.api_error(e);
        });
        emojis.forEach((e) => {
            message.react(e).catch((e) => {
                global_context.logger.api_error(e);
            });
        });

        const filter = (reaction, user) => emojis.includes(reaction.emoji.name) === true && user.id !== message.author.id;
        var collector = message.createReactionCollector({ filter: filter, time: remaining_time });
            ...
            switch (type) {
                case 0:
                    if (data.claimed_users.includes(u.id) === false && emoji === "🎁") {
                        ...
                        channel.send("<@" + u.id + "> claimed the gift~ (Gifted so far: " + data.claimed_users.length + ")").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    }
                    break;
                ...
                case 2:
                    if (data.lost_users.includes(u.id) === false && emoji === "🎰") {
                        let c = Math.floor(Math.random() * (100 - 0 + 1) + 0);
                        let amount = data.bets.has(u.id) === false ? 300 : data.bets.get(u.id);
                        let won = data.winnings.has(u.id) === false ? 0 : data.winnings.get(u.id);
                        if (c > 50) {
                            reacted_config.credits += amount;
                            global_context.neko_modules_clients.ssm.server_edit.edit(global_context, { type: "global_user", user: reacted_config });

                            data.bets.set(u.id, amount * 1.5);
                            data.winnings.set(u.id, won + amount);

                            channel.send("<@" + u.id + "> won against the neko!~ (Won so far: " + (won + amount) + "$)").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        } else {
                            data.lost_users.push(u.id);

                            channel.send("<@" + u.id + "> lost against the neko~ (Won: " + won + "$)").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
                ...
                case 3:
                    if (emoji === "💸") {
                        if (reacted_config.credits >= 50) {
                            ...
                            channel.send("<@" + u.id + "> donated to the neko!~ (Donated so far: " + data.donated + "$)").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
                ...
                case 4:
                    if (emoji === "😺") {
                        ...
                        u.send("You have joined the `smiling cats 😺` clan~").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        r.remove();
                    } else if (emoji === "😿") {
                        data.clans.set(u.id, 1);
                        u.send("You have joined the `crying cats 😿` clan~").catch((e) => {
                            global_context.logger.api_error(e);
                        });
                        r.remove();
                    }
                ...
                case 5:
                    if (data.claimed_users.includes(u.id) === false && emoji === "🌌") {
                        if (reacted_config.rep >= 10) {
                            ...
                            channel.send("<@" + u.id + "> passed the interview~ (Passed so far: " + data.claimed_users.length + ")").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
                ...
                case 6:
                    if (data.lost_users.includes(u.id) === false && emoji === "🗡️") {
                        ...
                        if (c > 80) {
                            ...
                            data.lost_users.push(u.id);
                            channel.send("<@" + u.id + "> won against the neko assassin!~ Congrats~").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        } else {
                            data.lost_users.push(u.id);
                            channel.send("<@" + u.id + "> lost against the neko assassin~").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
                ...
                case 8:
                    if (data.joined_users.includes(u.id) === false && emoji === "🧪") {
                        ...
                        if (c > 70) {
                            ...
                            data.joined_users.push(u.id);
                            channel.send("The neko scientist was satisfied with the results and rewarded <@" + u.id + ">~").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        } else {
                            data.joined_users.push(u.id);
                            channel.send("The experiment went wrong and <@" + u.id + "> was taken to the hospital~").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
                ...
                case 9:
                    if (emoji === "💵") {
                        if (reacted_config.credits >= 200) {
                            ...
                            reacted_config.inventory.push({ id: global_context.modules.crypto.randomBytes(16).toString("hex"), user_ID: u.id, item_ID: itemID });
                            global_context.neko_modules_clients.ssm.server_edit.edit(global_context, { type: "global_user", user: reacted_config });

                            channel.send("<@" + u.id + "> bought " + item.displayName + "!~").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
                ...
                case 10:
                    if (emoji === "❤️") {
                        ...
                        if (targetIndex != -1) {
                            ...
                            channel.send("<@" + u.id + "> sold " + item.displayName + " for 75$!~").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
                ...
                case 11:
                    if (emoji === "🎟️") {
                        let amount = data.user_tickets.has(u.id) === false ? 0 : data.user_tickets.get(u.id);
                        if (reacted_config.credits >= 100) {
                            reacted_config.credits -= 100;
                            global_context.neko_modules_clients.ssm.server_edit.edit(global_context, { type: "global_user", user: reacted_config });

                            data.user_tickets.set(u.id, amount + 1);
                            data.tickets.push(u.id);
                            channel.send("<@" + u.id + "> bought a ticket (Total: " + (amount + 1) + ")!~ (Bought so far: " + data.tickets.length + ")").catch((e) => {
                                global_context.logger.api_error(e);
                            });
                        }
                    }
