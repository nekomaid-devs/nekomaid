module.exports = {
    async refresh_status(global_context) {
        if (global_context.bot.neko_data.shards_ready === false) {
            return;
        }
    ...
    async refresh_website(global_context) {
        if (global_context.bot.neko_data.shards_ready === false || global_context.config.nekomaid_API_update_stats === false) {
            return;
        }
        ...
        await global_context.bot.shard
            .broadcastEval((client) => {
                return client.neko_data.vm_connections;
            })
            .then((results) => {
        ...
        let command_list = Array.from(global_context.commands.values())
            .filter((e) => {
    ...
    async refresh_bot_list(global_context) {
        if (global_context.config.dev_mode === true || global_context.config.nekomaid_API_update_bot_lists === false) {
            return;
        }
