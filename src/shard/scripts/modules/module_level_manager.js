    ...
    async update_server_level(command_data, xp_to_add, send_levelup_message = true) {
        ...
        let process_ranks = async () => {
            if (command_data.server_config.module_level_ranks.length > 0 && command_data.msg.guild.me.permissions.has(Permissions.FLAGS.MANAGE_ROLES) === false) {
                let channel = await command_data.global_context.bot.channels.fetch(command_data.server_config.module_level_levelup_messages_channel).catch((e) => {
            ...
            command_data.server_config.module_level_ranks.forEach((rank) => {
                let role = command_data.msg.guild.roles.cache.find((r) => {
                ...
                if (role !== undefined && processing_roles.includes(role.id) === false) {
                    if (rank.level <= command_data.tagged_server_user_config.level) {
                        command_data.tagged_member.roles.add(role).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
                    ...
                    if (command_data.tagged_server_user_config.level < rank.level) {
                        command_data.tagged_member.roles.remove(role).catch((e) => {
                            command_data.global_context.logger.api_error(e);
                        });
        ...
        if (start_level !== command_data.tagged_server_user_config.level) {
            let granted_roles_text = granted_roles.reduce((acc, curr) => {
                acc += "`" + curr.toString() + "`, ";
                return acc;
            }, "");
            ...
            let removed_roles_text = removed_roles.reduce((acc, curr) => {
                acc += "`" + curr.toString() + "`, ";
                return acc;
            }, "");
            ...
            if (command_data.server_config.module_level_levelup_messages == true && send_levelup_message == true) {
                ...
                let channel = await command_data.msg.guild.channels.fetch(command_data.server_config.module_level_levelup_messages_channel).catch((e) => {
                    command_data.global_context.logger.api_error(e);
                });
                if (channel !== undefined) {
                    channel.send(levelup_message + rank_message).catch((e) => {
                        command_data.global_context.logger.api_error(e);
                    });
                }
            }
