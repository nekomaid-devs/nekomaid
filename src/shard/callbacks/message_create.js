...
module.exports = {
    ...
    async process(global_context, message) {
        ...
        let tagged = message.mentions.members.size > 0;
        ...
        let command_data = {
            ...
            tagged_users: tagged ? [...message.mentions.users.values()] : [message.author],
            tagged_user: tagged ? message.mentions.users.first() : message.author,

            tagged_members: tagged ? [...message.mentions.members.values()] : [message.member],
            tagged_member: tagged ? message.mentions.members.first() : message.member,
        ...
        if (message.content === `<@!${global_context.bot.user.id}>`) {
            message.channel.send(`Prefix on this server is \`${command_data.server_config.prefix}\`.`).catch((e) => {
                global_context.logger.api_error(e);
            });
            return;
        }
        ...
        if (message.content.toLowerCase() === `thanks <@!${global_context.bot.user.id}>` || message.content.toLowerCase() === "thanks nekomaid" || message.content.toLowerCase() === "thanks neko") {
            let responses = ["You're welcome~ I guess- >~<", "W-What did I do?~", "No problem~ >//<", "You're welcome~ TwT"];
        ...
        if (global_context.config.sentry_enabled === true) {
            ...
            global_context.modules.Sentry.configureScope((scope) => {
        ...
        command_data.tagged_server_user_config =
            message.mentions.users.size < 1
                ? command_data.author_server_user_config
                : await global_context.neko_modules_clients.ssm.server_fetch.fetch(global_context, { type: "server_user", server_ID: message.guild.id, user_ID: message.mentions.users.first().id });
        command_data.tagged_user_config =
            message.mentions.users.size < 1 ? command_data.author_config : await global_context.neko_modules_clients.ssm.server_fetch.fetch(global_context, { type: "global_user", id: message.mentions.users.first().id });
        ...
        let tagged_user_tags = [...message.mentions.users.values()].reduce((acc, curr) => {
            acc += curr.tag + ", ";
            return acc;
        }, "");
