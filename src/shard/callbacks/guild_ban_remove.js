    ...
    async process(global_context, guild, user) {
        ...
        if (server_config.audit_bans == true && server_config.audit_channel !== "-1") {
            ...
            if (channel !== undefined) {
                let audit = await guild.fetchAuditLogs().catch((e) => {
                    global_context.logger.api_error(e);
                });
                ...
                if (last_audit.action === "MEMBER_BAN_REMOVE" && last_audit.target.id === user.id) {
                    ...
                    if (last_audit.executor.id === global_context.bot.user.id) {
                        executor = await global_context.bot.users.fetch(moderation_action.moderator).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    } else {
                        executor = await global_context.bot.users.fetch(last_audit.executor.id).catch((e) => {
                            global_context.logger.api_error(e);
                        });
                    }
                    ...
                    channel.send({ embeds: [embedBan] }).catch((e) => {
                        global_context.logger.api_error(e);
                    });
                }
            }
        ...
        let previous_ban = server_bans.find((ban) => {
            return ban.user_ID === user.id;
        });
