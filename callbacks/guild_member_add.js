module.exports = {
    ...
    async process(global_context, member) {
        ...
        let mute_role = await member.guild.roles.fetch(server_config.mute_role_ID).catch(e => { global_context.logger.api_error(e); });
        if(mute_role !== undefined) {
            if(server_mutes.some(e => { return e.user_ID === member.user.id; })) {
        ...
        if(server_config.welcome_messages == true) {
            ...
            let member_display_name = server_config.welcome_messages_ping ? `${member}` : "**" + member.user.tag + "**";
            format = format.replace("<user>", member_display_name);

            let channel = await global_context.bot.channels.fetch(server_config.welcome_messages_channel).catch(e => { global_context.logger.api_error(e); });
