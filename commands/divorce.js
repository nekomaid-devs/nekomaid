module.exports = {
    ...
    async execute(command_data) {
        if(command_data.msg.mentions.users.array().length > 0) {
            ...
            let married_user = await command_data.global_context.bot.users.fetch(command_data.tagged_user_config.marriedID).catch(e => { command_data.global_context.logger.api_error(e); });
        ...
        let married_user = await command_data.global_context.bot.users.fetch(command_data.author_config.marriedID).catch(e => { command_data.global_context.logger.api_error(e); });
        let user_config = -1;
        if(married_user !== undefined) {
