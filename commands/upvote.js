module.exports = {
    ...
    execute(data) {
        ...
        var end = new Date();
        var start = new Date(data.authorConfig.lastUpvotedTime4);
        var start2 = new Date(data.authorConfig.lastUpvotedTime0);
        
        var endNeeded = new Date(start.getTime() + (3600000 * 12));
        var timeLeft = endNeeded - end;
        var voteIn0 = timeLeft <= 0 ? "now" : "in " + data.bot.tc.convertTime(timeLeft)
